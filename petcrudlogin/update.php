...
if (isset($_POST["submit"])) {
    ...
    if ($pictureArray->error === 0) {       
        ...
        $sql = "UPDATE `user` SET `first_name`=?,`last_name`=?,`email`=?,`phone_number`=?,`address`=?, picture=? WHERE `id`=?";
        $db->query($sql, array($f_name, $l_name, $email, $phone, $address, $pictureArray->fileName,$id));
    } else {
        $sql = "UPDATE `user` SET `first_name`=?,`last_name`=?,`email`=?,`phone_number`=?,`address`=? WHERE `id`=?";
...
<div class="container">
    <div class="<?php echo $class; ?>" role="alert">
        ...
        <h2>Update</h2>        
        <img class='userImage rounded-circle' src='pictures/<?php echo $data['picture'] ?>' alt="<?php echo "$data[first_name] $data[last_name]" ?>">
            ...
            <div class="table-responsive mx-auto table-width">
                <table class='table table-hover table-striped'>
                    ...
                    <tr>
                        ...
                        <td><button name="submit" class="btn btn-success" type= "submit"><i class="fas fa-save"></i></button></td>
                        <td><a href= "<?php echo $backBtn?>"><button class="btn btn-warning" type="button"><i class="fas fa-hand-point-left"></i></button></a></td>
                    </tr>
                </table>
            </div>
        </form>    
